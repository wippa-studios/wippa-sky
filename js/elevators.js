...
export function drawElevatorCar(ctx, car, shaft, state, isNight) {
  ...
  // The car's screen Y is derived from its fractional floor position
  const towerLeft = (state.canvasW - state.cols * CELL_W * zoom) / 2;
  const baseY = (state.canvasH || 600) - 140; // 140 = GROUND_Y_OFFSET

  const cellX = towerLeft + col * CELL_W * zoom;
  const cellW = CELL_W * zoom;
  const cellH = CELL_H * zoom;

  // car Y: cell is drawn at floor=floorPos (integer), car slides between floors
  // row = Math.floor(floorPos) — screen y of the integer floor cell
  const rowBelow = Math.floor(floorPos);
  const frac = floorPos - rowBelow;

  const yBelow = baseY - (rowBelow + 1) * cellH + state.cameraY;
  const yAbove = yBelow - cellH;
  const carY = yBelow + frac * cellH;
  const carX = cellX;
  const carW = cellW;
  const carH = cellH;
  ...
  grad.addColorStop(0, isNight ? '#4a5568' : '#64748b');
  grad.addColorStop(0.3, isNight ? '#718096' : '#94a3b8');
  grad.addColorStop(0.5, isNight ? '#8292a6' : '#b0bec5');
  grad.addColorStop(0.7, isNight ? '#718096' : '#94a3b8');
  grad.addColorStop(1, isNight ? '#4a5568' : '#64748b');
  ...
  const count = (car.passengers && car.passengers.length) || 0;
...
export function drawElevatorShafts(ctx, state) {
  ...
  for (const [row, cells] of state.grid) {
    for (let col = 0; col < state.cols; col++) {
      ...
      // shaft background (darker than regular cells)
      drawRoundedRect(ctx, x, y, w, h, 2);
      const shaftGrad = ctx.createLinearGradient(x, y, x, y + h);
      shaftGrad.addColorStop(0, '#0f1520');
      shaftGrad.addColorStop(0.5, '#151d2c');
      shaftGrad.addColorStop(1, '#0f1520');
  ...
  if (state._callButtons) {
    ...
    for (const [key, call] of state._callButtons) {
      ...
      const pos = cellToScreenLocal(state, call.floor, shaft.col);
