...
import { getFloorCount, getCell } from './grid.js';
import { getSimCount } from './sims.js';
import { getMoodColor, getMoodBand, getCellHappiness, getCellWaitingStats } from './needs.js';
...
export function showInfoPanel(state, row, col) {
  ...
  title.textContent = `${ft.icon} ${ft.name} — ${row < 0 ? 'B' + Math.abs(row) : 'F' + (row + 1)}`;
  ...
  content.innerHTML = `
    ...
    <div class="info-row"><span class="label">Occupancy</span><span class="value">${cell.occupancy}/${ft.capacity}</span></div>
    <div class="info-row"><span class="label">Happiness</span><span class="value" style="color:${moodColor}">${Math.round(happiness)}% (${getMoodBand(happiness)})</span></div>
    <div class="info-row"><span class="label">Income Mult</span><span class="value">${(cell.incomeMult * 100).toFixed(0)}%</span></div>
    ${cell.type === 'office' ? `<div class="info-row"><span class="label">Staffing</span><span class="value">${cell.staff || 0}/${ft.staffCapacity}</span></div>` : ''}
    ${waiting.count > 0 ? `<div class="info-row"><span class="label">Waiting for Elev</span><span class="value" style="color:#f87171">${waiting.count} sims (avg ${Math.round(waiting.avgWait)}s)</span></div>` : ''}
    <div class="info-row"><span class="label">Built</span><span class="value">Day ${cell.built}</span></div>
    ${cell.vacant ? `<div class="info-row"><span class="label">Status</span><span class="value" style="color:#f87171">VACANT (Day ${cell.vacantSince})</span></div>` : ''}
  `;
