...
import { DEFAULT_COLS, LAND_PURCHASE_DEFAULT } from './constants.js';
...
export function screenToCell(state, sx, sy, canvasH, groundYOffset) {
  const towerLeft = (state.canvasW - state.cols * 80 * state.zoom) / 2;
  const baseY = canvasH - groundYOffset;
  const col = Math.floor((sx - towerLeft) / (80 * state.zoom));
  const row = Math.floor((baseY - sy + state.cameraY) / (40 * state.zoom));
