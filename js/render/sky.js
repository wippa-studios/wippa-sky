// ═══════════════════════════════════════════════════════════
//  Wippa Sky v2 — Sky, Weather & Background City Renderer
// ═══════════════════════════════════════════════════════════
...
export function drawSky(ctx, state, canvasW, canvasH) {
  ...
  drawBackgroundCity(ctx, state, canvasW, canvasH);
...
function drawSkyGradient(ctx, state, canvasW, canvasH) {
  const p = state.dayTime;
  let stops;
  if (p < 0.15) stops = SKY_COLORS.dawn;
  else if (p < 0.5) stops = SKY_COLORS.day;
  else if (p < 0.7) stops = SKY_COLORS.dusk;
  else stops = SKY_COLORS.night;
...
function drawSunMoon(ctx, state, canvasW, canvasH) {
  ...
  if (p > 0.12 && p < 0.72) {
    ...
    sg.addColorStop(0, 'rgba(255,250,205,1)');
    sg.addColorStop(0.25, 'rgba(255,218,113,.92)');
    sg.addColorStop(1, 'rgba(255,174,75,0)');
    ctx.fillStyle = sg;
    ctx.beginPath();
    ctx.arc(orbX, orbY, 45, 0, Math.PI * 2);
    ctx.fill();

    const gg = ctx.createRadialGradient(orbX, orbY, 35, orbX, orbY, 150);
    gg.addColorStop(0, 'rgba(255,230,60,0.12)');
    ...
    mg.addColorStop(0, 'rgba(220,230,250,1)');
    ...
    mgl.addColorStop(0, 'rgba(200,210,230,0.08)');
...
export function drawBackgroundCity(ctx, state, canvasW, canvasH) {
  const isNight = state.dayPhase === 'night' || state.dayPhase === 'dusk';

  for (let i = 0; i < state.bgBuildings.length; i++) {
    const b = state.bgBuildings[i];
    const depth = b.layer === 0 ? 0.42 : 0.68;
    const by = canvasH - GROUND_Y_OFFSET - b.h * depth;

    const bg = ctx.createLinearGradient(b.x, by, b.x, by + b.h);
    bg.addColorStop(0, b.color);
    bg.addColorStop(1, 'rgba(4,13,29,.88)');
    ctx.fillStyle = bg;
    ctx.beginPath();
    ctx.roundRect(b.x, by, b.w, b.h, 2);
    ctx.fill();

    ctx.fillStyle = b.color;
    if (b.layer === 1 && b.w > 36) {
      ctx.fillRect(b.x + b.w * 0.32, by - 8, b.w * 0.36, 8);
      ctx.fillRect(b.x + b.w * 0.47, by - 18, 2, 18);
    }

    if (b.windows) {
      const ww = 3;
      const wh = 4;
      const gapX = b.w / (b.windowCols + 1);
      const gapY = b.h / (b.windowRows + 1);
      ...
      for (let wr = 0; wr < b.windowRows; wr++) {
        for (let wc = 0; wc < b.windowCols; wc++) {
          ...
          if (lit && b.layer === 1) {
            ctx.fillStyle = 'rgba(255,197,112,.08)';
...
function drawHaze(ctx, state, canvasW, canvasH) {
  const haze = ctx.createLinearGradient(0, canvasH - GROUND_Y_OFFSET - 130, 0, canvasH - GROUND_Y_OFFSET + 15);
  haze.addColorStop(0, 'rgba(174,212,226,0)');
  haze.addColorStop(1, 'rgba(174,212,226,.1)');
  ctx.fillStyle = haze;
  ctx.fillRect(0, canvasH - GROUND_Y_OFFSET - 130, canvasW, 145);
}
