...
export function calculateExpenses(state) {
  ...
  for (const [, cells] of state.grid) {
    for (let c = 0; c < state.cols; c++) {
      const cell = cells[c];
      if (cell && cell.type === 'service') {
        serviceDiscount += SERVICE_EXPENSE_REDUCTION;
      }
    }
  }
  serviceDiscount = Math.min(serviceDiscount, SERVICE_EXPENSE_REDUCTION * MAX_SERVICES);
  return Math.floor(base * (1 - serviceDiscount));
}
...
export function calculateSatisfaction(state) {
  ...
  const avgMood = state.tenants.reduce((s, t) => s + t.mood, 0) / state.tenants.length;
  let amenityScore = 0;
  let amenityCount = 0;
  for (const [row, cells] of state.grid) {
    if (row <= 0) continue;
  ...
  const amenityBonus = amenityCount > 0 ? Math.min(amenityScore * 0.3, 20) : 0;
  const socialPulse = (avgMood - 70) * 0.15;
  return Math.min(100, Math.max(0, Math.floor(avgMood * 0.6 + amenityBonus + 30 + socialPulse)));
}
